// PROTOTYPE CODE — see the package comment in main.go.
//
// These handlers take every request at face value: no auth, no authorization
// checks, no validation of decoded bodies, no request-size limits, and most
// multi-statement writes are not wrapped in a transaction. Errors are returned
// verbatim to the caller so the model is easy to debug by hand. All of that is
// fine for a single-user localhost explorer and unacceptable anywhere else.
package main

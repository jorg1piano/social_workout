# Model explorer (Go web app) — ⚠️ PROTOTYPE

> **This is a throwaway prototype, not production code.** It exists to *feel out*
> the plan-vs-record data model in `sqlite/schema.sql` by clicking through it on
> a laptop. Do not deploy it, do not expose it to a network, and do not treat it
> as the shape of the real backend — expect it to be deleted or rewritten
> wholesale.

A tiny local web app for exploring the plan-vs-record data model: single user,
single SQLite file, no auth.

Deliberately missing, because it is a prototype:

- **No authentication, authorization, or multi-user support** — every request is
  trusted, and everyone shares one database.
- **No input validation and no request-size limits** — bodies are decoded
  straight into structs and handed to SQL.
- **Raw internal errors are returned to the client**, SQL error text included.
- **No transactions around multi-statement writes**, so a mid-write failure can
  leave partial rows behind.
- **No tests, no migrations, no observability** — the DB is bootstrapped by
  executing the canonical `.sql` files verbatim on first run.
- **Not built for concurrency** — the pool is pinned to a single connection.
...
On first run it creates `backend/app.db` and bootstraps it from the **canonical**
